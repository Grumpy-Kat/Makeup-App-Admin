...
XYZColor LabtoXYZ(LabColor color) {
  ...
  double xPow = pow(x, 3).toDouble();
  if(xPow > CIE_E) {
    x = xPow;
  } else {
    x = (x - 16.0 / 116.0) / 7.787;
  }
  double yPow = pow(y, 3).toDouble();
  if(yPow > CIE_E) {
    y = yPow;
  } else {
    y = (y - 16.0 / 116.0) / 7.787;
  }
  double zPow = pow(z, 3).toDouble();
  if(zPow > CIE_E) {
    z = zPow;
  } else {
...
LabColor XYZtoLab(XYZColor color) {
  ...
  if(x > CIE_E) {
    x = pow(x, (1.0 / 3.0)).toDouble();
  } else {
    x = (7.787 * x) + (16.0 / 116.0);
  }
  if(y > CIE_E) {
    y = pow(y, (1.0 / 3.0)).toDouble();
  } else {
    y = (7.787 * y) + (16.0 / 116.0);
  }
  if(z > CIE_E) {
    z = pow(z, (1.0 / 3.0)).toDouble();
  } else {
...
XYZColor RGBtoXYZ(RGBColor color, { targetIlluminant = '' }) {
  ...
  for(int i = 0; i < values.length; i++) {
    if(values[i] <= 0.04045) {
      ...
      linearChannels.add(pow((values[i] + 0.055) / 1.055, 2.4).toDouble());
    }
  }
